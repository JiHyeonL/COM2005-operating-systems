/*
 * Copyright(c) 2023 All rights reserved by 소프트웨어학부 2019007329 Jihyeon Lee.
 ...
 * arg는 사용하지 않고 하나의 스레드가 한번에 9개 행 검사한다.
 */
 ...
 * arg는 사용하지 않고 하나의 스레드가 한번에 9개 열 검사한다. 
 */
 ...
 * arg로 하나의 격자를 가져와서 하나의 스레드로 검사한다.
 */
...
{              
    ...
    for (i = i_set; i < i_set+3; i++) {
    	for (j = j_set; j < j_set+3; j++) {
    		index = sudoku[i][j]-1;
    		sub[index] += 1;
    		if (sub[index] > 1) {
    			valid[2][num] = 0;
...
void check_sudoku(void)
{
     ...
     */
   	...
   	for(i = 2; i < 11; i++) {
   		temp[i-2] = i-2;
		pthread_create(&tid[i], NULL, check_subgrid, &temp[i-2]);
	}
